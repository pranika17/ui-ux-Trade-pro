...
import "./globals.css";
import { Facebook, Instagram, Twitter, Linkedin,Send, Youtube} from "lucide-react";
    ...
    <div className="container mx-auto px-4">
      ...
      <div className="grid  grid-cols-1 md:grid-cols-4 gap-8" >
        <div>
          ...
          <p>Banglore-560103</p>  
          <p className="text-sm mb-2"><a href ="" className="text-blue-500 hover:underline ">Contact Us</a></p>
        ...
        <div>
          ...
          <ul className="space-y-2">{["Stocks","Futures & Options","IPO","Mutual Funds" ].map((item)=>(
            <li  className="text-sm hover:text-blue-500 cursor:pointer" key={item}>{item}</li>
          ))}</ul>
        </div>
        <div>
          <h3 className="font-semibold mb-4 ">TradePro</h3>
          <ul className="space-y-2">{["About Us","Pricing","Blog","Carrer","Help and Support" ].map((item)=>(
            <li className="text-sm hover:text-blue-500 cursor:pointer" key={item}>{item}</li>
          ))}</ul>
        </div>
        <div>
          <h3 className="font-semibold mb-4 ">Quick Links</h3>
          <ul className="space-y-2">{["AMc Mutual Funds","Calculators","Glossary","Tradepro Digest","Help and Support" ].map((item)=>(
            <li   className="text-sm hover:text-blue-500 cursor:pointer" key={item}>{item}</li>
          ))}</ul>
        </div>
        </div>
        <div className="mt-8 pt-8 border-gray-200 flex flex-col md:flex-row justify-between items-center">
          <p className="text-sm text-gray-500 mb-4 mdLmb-0">
            {""}
          © 2016-2024 TradePro. All rights reserved, Built with ❤️️ in India
          </p>
